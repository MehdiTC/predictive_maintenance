...
A production-style predictive-maintenance ML platform that predicts turbine Remaining Useful Life
(RUL) from streaming sensor data and issues maintenance alerts — built end to end on public NASA
...
**Live demo:** <https://turbine-guard-web.onrender.com> · **One-minute overview:**
[docs/portfolio.md](docs/portfolio.md) · **Architecture:** [docs/architecture.md](docs/architecture.md)
· **Model card:** [docs/model_card.md](docs/model_card.md)

## What it does

TurbineGuard runs a complete MLOps loop on the NASA C-MAPSS dataset:

* Acquires and versions NASA C-MAPSS run-to-failure sensor data with checksummed provenance.
* Builds leakage-safe time-series features shared identically between training and serving.
* Trains and compares four model families, selecting a champion by explicit operational criteria
  with calibrated prediction intervals.
* Serves predictions and maintenance-risk alerts through a versioned FastAPI service on PostgreSQL.
* Replays held-out engine trajectories as a live sensor stream with delayed failure labels.
* Monitors data quality, drift, and online performance, and retrains behind gated promotion.
* Ships a server-rendered dashboard and a zero-cost public live demo.

## Results at a glance

All figures are on public/simulated NASA data and are regenerated by `make reproduce`. Maintenance
costs are normalized, hypothetical units — not currency or claimed savings. See
[docs/model_card.md](docs/model_card.md) for full metrics and provenance.

| Metric (held-out replay engines) | Value |
| --- | --- |
| Champion | Ridge (`alpha=1`), RUL capped at 125, chosen by simplicity-tolerance policy |
| RUL error | MAE ≈ 10.5, RMSE ≈ 14.0 cycles |
| Conformal interval coverage | 0.898 empirical at nominal 90% |
| Critical-alert recall / false alarms | ≈ 0.77 / < 1 per 1,000 cycles |
| Simulated predictive vs reactive cost | −64.3% per engine, 0 missed failures (base scenario) |

## Project status

The project was built in 13 bounded implementation loops (0–12). The design, per-loop rationale, and
final state are documented in [docs/architecture.md](docs/architecture.md),
[docs/portfolio.md](docs/portfolio.md), and the decision records under [docs/adr/](docs/adr/).

**Loops 0–12 are complete.** The offline pipeline, MLflow tracking/registry, FastAPI inference on
PostgreSQL, held-out replay with delayed feedback, monitoring and gated retraining, the
containerized stack, and CI share one non-root production image and a health-ordered Compose
topology. GitHub Actions covers Python quality, real PostgreSQL migrations/integration, local
MLflow, image contracts, and a deterministic end-to-end API smoke test. The server-rendered
dashboard and a zero-cost public deployment (free Render web service, Neon free PostgreSQL, and an
immutable checksum-pinned deployment bundle) are live. The only remaining portfolio asset is a
hand-recorded GIF/video of the live demo.
...
make reproduce           # acquire -> process -> features -> train (regenerates all metrics)
```

This downloads the public NASA archive, validates and processes it, builds the feature layer, and
trains/evaluates every model family into `data/models/cmapss/FD001/reports/`. No live demo, no
account, and no Docker required for this path.
...
| `make reproduce`    | Acquire → process → features → train (all offline metrics) |
...
The dashboard and demo are **live** at <https://turbine-guard-web.onrender.com>. `render.yaml`
defines the zero-cost public topology (ADR 0011): one free Render web service backed by an external
Neon free-tier PostgreSQL database. The public champion is an immutable, exported, checksum-pinned
deployment bundle (`scripts/deployment_bundle.py export`) restored and re-verified at cold start; no
live MLflow service runs publicly, and the demo cannot mutate a registry. The full topology with
live MLflow remains the local Docker Compose stack. Startup never trains. See
...
* [docs/portfolio.md](docs/portfolio.md) — one-minute overview, resume bullets, interview points,
  and one-command reproduction.
* [docs/architecture.md](docs/architecture.md) — component overview, data-flow and request sequence
  diagrams, deployment topologies.
* [docs/model_card.md](docs/model_card.md) — champion model card with final metrics and provenance.
* [docs/limitations.md](docs/limitations.md) — consolidated limitations and known risks.
* [docs/scaling.md](docs/scaling.md) — how the system would scale, and why heavier tooling is
...
├── docs/
│   ├── architecture.md          # component + sequence diagrams, deployment topologies
│   ├── portfolio.md             # one-minute overview, resume/interview material
│   ├── model_card.md            # committed champion model card + final metrics
│   ├── limitations.md
│   ├── scaling.md
│   ├── data_contract.md
│   ├── features.md
